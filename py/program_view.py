...
class Program_view:
	...
	def addItemToTree(self, treeview, array, parent, suffix =""):
		itmes = {}
		for item in array:
			items[item] = self.treeview.insert(parent, 'end', text= str(item) + suffix)
		return items

	def linedata_to_treeview(self, AllStations, AllGates, AllRobots, AllTools, AllSIOs, AllEstops, AllLCs, AllAccLCs, AllOPs, AllSPDs, AllICSDs, AllDrives, zonelist):
		for zone in zonelist:
			item_zone = self.treeview.insert('', 'end', text=zone)
			addItemToTree(self.treeview, AllGates[zone], item_zone)
			station_items = addItemToTree(self.treeview, AllStations[zone], item_zone)
			for station in AllStations[zone]:
				addItemToTree(self.treeview, AllSIOs[zone][station], station_items[station])
				addItemToTree(self.treeview, AllEstops[zone][station], station_items[station], " Estop")
				addItemToTree(self.treeview, AllLCs[zone][station], station_items[station], " LC")
				addItemToTree(self.treeview, AllAccLCs[zone][station], station_items[station], " AccLC")
				addItemToTree(self.treeview, AllOPs[zone][station], station_items[station], " OP")
				addItemToTree(self.treeview, AllSPDs[zone][station], station_items[station], " SPD")
				addItemToTree(self.treeview, AllICSDs[zone][station], station_items[station], " ICSD")
				addItemToTree(self.treeview, AllDrives[zone][station], station_items[station])