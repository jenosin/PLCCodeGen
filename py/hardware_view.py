...
class Hardware_view:
	...
	def __init__(self, tab, controller):
		...
		self.module_grid.rowconfigure(0, weight=0)  # 第一行(row=0)的权重设为1，自动延伸
		self.module_grid.rowconfigure(1, weight=1)  # 第二行(row=1)的权重设为0，固定大小
		self.module_grid.rowconfigure(2, weight=0)  # 第三行(row=2)的权重设为0，固定大小
		self.tree_grid = ttk.Frame(tab,borderwidth=2, relief="groove")
		self.tree_grid.grid(row=0, column=1, columnspan=2,padx=10, pady=10, sticky="nse")
		self.info_grid = ttk.Frame(tab,borderwidth=2, relief="solid")
		self.info_grid.grid(row=1,column=0,columnspan=2, padx=10, pady=10,sticky="ws")
		...
		#创建模块区域框架和内容
		self.frame = ttk.Frame(self.module_grid, width=350, height=400, borderwidth=0, relief="groove")
		self.frame.pack(side="left", fill="both", expand=True)
		self.frame.pack_propagate(False)
		self.dropdown = tk.StringVar()
		self.dropdown.set("选择模块")
		self.dropdown_menu = tk.OptionMenu(self.frame, self.dropdown, *self.controller.module_list, command=self.controller.dropdown_on_select)
		self.dropdown_menu.config(width=20,height=1)
		self.dropdown_menu.grid(row=0,column=0,padx=10,pady=5,sticky="w")
		self.import_button = tk.Button(self.frame, text="Import IP List", command=self.controller.on_import)
		self.import_button.grid(row=0,column=1,padx=10,pady=5,sticky="e")
		...
		self.save_button.grid(row=2,column=1,padx=10,pady=5,sticky="e")
		...
		self.tree_frame.rowconfigure(0, weight=1)  # 第一行(row=0)的权重设为1，自动延伸
		self.tree_frame.rowconfigure(1, weight=0)  # 第二行(row=1)的权重设为0，固定大小
		self.tree_frame.rowconfigure(2, weight=0)  # 第三行(row=2)的权重设为0，固定大小
		self.treeview_frame = ttk.Frame(self.tree_frame, width=350, height=400,borderwidth=1, relief="groove")
		self.treeview_frame.grid(row=0, column=0, columnspan=2, padx=10, pady=5,sticky="nsew")
		...
		self.load_tree_button.grid(row=2, column=1, padx=10, pady=5,sticky="e")   
